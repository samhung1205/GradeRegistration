...
> 供助教設計的輕量成績登記 SPA（Single-Page Application），在瀏覽器端運行，無需登入。
...
- 設定會同步到 Supabase（`app_settings`），並在本機 `localStorage` 保留快取
...
右上角下拉選單切換「助教名」（或自訂名稱）。
選擇後自動存入 `localStorage`（裝置偏好），每次成績登記自動帶入登記人欄位。
...
目標：**取得一個公開網址 → 把網址傳給助教 → 對方用瀏覽器打開就能用**。
...
5. **複製這個網址**，傳給另一位助教即可。對方用瀏覽器打開就能用，兩人共用同一份資料。

### 四、之後你改網站內容

- 改完 `index.html` 後，在專案資料夾再執行一次 **`npx vercel`**，會**更新同一個網址**的內容。
- 其他人**不用換網址**，重新整理頁面（F5）就會看到最新版。
